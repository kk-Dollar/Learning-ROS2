...
{
public:
  CallAction(const std::string& action_name, const BT::NodeConfig& conf) : BT::StatefulActionNode(action_name, conf)
  {
  ...
  BT::NodeStatus onRunning()
  {
    if (!server_called_)
    {
      ...
      if (rclcpp::spin_until_future_complete(node2_, send_goal_future) != rclcpp::FutureReturnCode::SUCCESS)
      {
  ...
  static BT::PortsList providedPorts()
  {
    return { BT::InputPort<std::string>("server_name"), BT::InputPort<float>("x"), BT::InputPort<float>("y"),
