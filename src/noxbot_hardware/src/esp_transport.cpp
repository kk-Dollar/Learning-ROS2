...
EspTransport::EspTransport(rclcpp::Node* node)
  : node_(node)
  ,  // these all are initiliser which is updated before the constructor call
  left_vel_(0.0)
  , right_vel_(0.0)
  , left_pos_(0.0)
  , right_pos_(0.0)
  ...
  , initialized_(false)
{
  ...
  fb_sub_ = node_->create_subscription<std_msgs::msg::Float32MultiArray>(
      "/wheel_feedback", 10, std::bind(&EspTransport::feedbackCallback, this, std::placeholders::_1));
}
...
void EspTransport::sendWheelCommands(double left_rad_s, double right_rad_s)
{
  ...
  msg.data.resize(2);
  msg.data[0] = static_cast<float>(left_rad_s);
  msg.data[1] = static_cast<float>(right_rad_s);

  cmd_pub_->publish(msg);
}
void EspTransport::feedbackCallback(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
{
  if (msg->data.size() < 4)
  {
